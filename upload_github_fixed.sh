...
# LUX IMAGE SEARCH GitHub 업로드 스크립트 (오류 수정 버전)
# Repository: https://github.com/eonyeon/image-search

echo "🚀 LUX IMAGE SEARCH GitHub 업로드 (수정된 Actions 포함)"
...
# 5. 커밋
echo -e "\n${YELLOW}💾 커밋 생성...${NC}"
COMMIT_MSG="fix: GitHub Actions workflows for macOS and Windows builds"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "\n${GREEN}📋 GitHub Actions 수정 내용:${NC}"
    echo "1. macOS: universal-apple-darwin → x86_64 + aarch64 분리"
    echo "2. Windows: 자동 릴리즈 제거, artifacts만 업로드"
    echo "3. 수동 릴리즈 워크플로우 추가 (release.yml)"
