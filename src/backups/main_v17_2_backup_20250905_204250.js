...
class FashionSearchHybrid {
    ...
    async init() {
        this.addDebugLog('🚀 Fashion Search v17.2 - 하이브리드 수정판', 'critical');
    ...
    createDebugPanel() {
        ...
        controlPanel.innerHTML = `
            <h3 style="margin-top: 0;">🔍 v17.2 Fixed</h3>
    ...
    async extractHybridFeatures(imageData, canvas) {
        ...
        // 4. 형태 특징 (32차원) - canvas 대신 imageData 사용
    ...
    extractColorHistogram(imageData) {
        ...
        return features.slice(0, 256); // 정확히 256차원
    }
    ...
    extractEdgeFeatures(imageData) {
        ...
        return features.slice(0, 128); // 정확히 128차원
    }
    ...
    // 형태 특징 (수정: canvas 대신 imageData 직접 사용)
    extractShapeFeatures(imageData) {
        ...
        return features.slice(0, 32); // 정확히 32차원
    }
    ...
    // 유사도 계산 (가중치 적용, NaN 방지)
    calculateWeightedSimilarity(vec1, vec2) {
        if (!vec1 || !vec2) return 0;
        if (vec1.length !== vec2.length) return 0;
        if (vec1.length !== 544) return 0; // 차원 검증
        ...
        // 가중 평균
        const totalSim = 
            (isNaN(colorSim) ? 0 : colorSim) * weights.color +
            (isNaN(edgeSim) ? 0 : edgeSim) * weights.edge +
            (isNaN(patternSim) ? 0 : patternSim) * weights.pattern +
            (isNaN(shapeSim) ? 0 : shapeSim) * weights.shape;
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🔍 Fashion Search v17.2 - Fixed');
