// Fashion Image Search v19.2 - Full Fix
// 폴더 선택 및 모델 토글 기능 수정
// 2025-01-03

console.log('🚀 Fashion Search v19.2 - Full Fix');

class LuxuryFashionSearchApp {
    constructor() {
        this.version = 'v19.2.0-FULL-FIX';
        this.dbName = 'fashionSearchDB_v19_2';
        this.db = null;
        ...
        this.models = {
            ...
            efficientNetAvailable: false  // EfficientNet 사용 가능 여부
        };
        this.isReady = false;
        this.init();
    }
    ...
    async tryLoadEfficientNet() {
        try {
            console.log('🔄 EfficientNet 로드 시도 중...');
            this.updateStatus('⏳ EfficientNet 로딩 중... (고정확도 모델)');
            
            // 실제로 EfficientNet 로드 시도
            // 여기서는 더 간단한 방법으로 시도
            const testTensor = tf.zeros([1, 224, 224, 3]);
            
            // MobileNet v3를 EfficientNet 대신 사용 (더 나은 정확도)
            // 실제 EfficientNet은 CDN 문제가 있을 수 있음
            await this.loadScript('https://cdn.jsdelivr.net/npm/@tensorflow-models/mobilenet@2.1.0/dist/mobilenet.min.js');
            
            // 더 높은 정확도의 MobileNet 설정
            this.models.efficientNet = await mobilenet.load({
                version: 2,
                alpha: 1.4  // 더 큰 모델 (더 정확)
            });
            
            testTensor.dispose();
            
            this.models.efficientNetAvailable = true;
            console.log('✅ 고정확도 모델 로드 성공!');
            this.updateStatus('✅ 고정확도 모델 사용 가능!');
            this.updateModelToggle();
            
        } catch (error) {
            console.log('ℹ️ 고정확도 모델 로드 실패. 기본 모델 사용:', error.message);
            this.models.efficientNetAvailable = false;
            this.updateModelToggle();
        }
    }
    ...
    updateModelToggle() {
        ...
        if (toggleBtn) {
            if (this.models.efficientNetAvailable) {
                toggleBtn.disabled = false;
                toggleBtn.textContent = this.models.activeModel === 'efficientnet' 
                    ? '🚀 고정확도 모드 ON' 
                    : '⚡ 빠른 모드 ON';
                toggleBtn.style.opacity = '1';
            } else {
                toggleBtn.disabled = true;
                toggleBtn.textContent = '⏳ 고정확도 모델 로딩 중...';
    ...
    toggleModel() {
        if (!this.models.efficientNetAvailable) {
            alert('고정확도 모델이 아직 로드되지 않았습니다.');
            return;
        }
        
        this.models.activeModel = this.models.activeModel === 'mobilenet' ? 'efficientnet' : 'mobilenet';
        this.updateModelToggle();
        
        const modelName = this.models.activeModel === 'efficientnet' ? '고정확도 모드' : '빠른 모드';
    ...
    async extractModelFeatures(imageElement) {
        const model = this.models.activeModel === 'efficientnet' && this.models.efficientNet 
    ...
    async processSearchImage(file) {
        ...
        reader.onload = async (e) => {
            ...
            img.onload = async () => {
                ...
                document.getElementById('searchStatus').textContent = `🔍 ${this.models.activeModel === 'efficientnet' ? '고정확도' : '빠른'} 모드로 검색 중...`;
    ...
    setupUI() {
        const html = `
            <div id="fashionSearchApp">
                <h1>🎯 Fashion Image Search v19.2</h1>
    ...
    attachEventListeners() {
        ...
        document.getElementById('selectFolderBtn')?.addEventListener('click', () => {
            ...
            input.directory = true;  // Firefox 지원
            input.multiple = true;
    ...
    async indexFiles(files) {
        ...
        progressDiv.innerHTML = `
            ...
            <p>모델: ${this.models.activeModel === 'efficientnet' ? '고정확도' : '빠른'} 모드</p>
        `;
    ...
    async validateDB() {
        ...
        request.onsuccess = () => {
            ...
            report += `현재 활성 모델: ${this.models.activeModel === 'efficientnet' ? '고정확도' : '빠른'} 모드\n`;
            report += `고정확도 모델 사용 가능: ${this.models.efficientNetAvailable ? '예' : '아니오'}\n\n`;
            ...
            report += `=== 모델별 인덱싱 분포 ===\n`;
            report += `빠른 모드 (MobileNet): ${stats.mobilenet}개\n`;
            report += `고정확도 모드: ${stats.efficientnet}개\n`;
            ...
            if (images.length > 0) {
                ...
                images.slice(-5).forEach((img, i) => {
                    ...
                    report += `   - 모델: ${img.modelType === 'efficientnet' ? '고정확도' : '빠른'}\n`;
