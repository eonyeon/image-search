...
// Fashion Search v11.1 - Enhanced Product Recognition (Fixed)
class EnhancedFashionSearch {
    constructor() {
        ...
        console.log(`🚀 Fashion Search v${this.version} - Enhanced Product Recognition (Fixed)`);
        console.log('✨ 버그 수정: HTML-JS ID 매칭 문제 해결');
        this.init();
    }
    ...
    setupEventListeners() {
        ...
        // DB 초기화 버튼 (HTML에서 clear-database-btn)
        ...
        if (clearDbBtn) {
            ...
            console.error('❌ clear-database-btn을 찾을 수 없습니다');
        }
    ...
    switchMode(mode) {
        ...
        // 모드별 컨텐츠 표시/숨김 (HTML에서는 mode-content 클래스 사용)
    ...
    async displayResults(results) {
        ...
        if (resultCount) {
            resultCount.textContent = `(${topResults.length}개)`;
        }
    ...
    async indexFolder(folderPath) {
        ...
        try {
            ...
            // UI 업데이트
            if (folderInfo && folderPathElement && imageCountElement) {
            ...
            const updateProgress = () => {
                ...
                if (progressText) progressText.textContent = `${processed} / ${images.length}`;
            ...
            if (indexResults && indexedCount) {
                indexedCount.textContent = this.imageDatabase.length;
    ...
    async clearDB() {
        ...
        if (indexResults) indexResults.style.display = 'none';
    ...
    // 콘솔 명령어
    version() {
        return `Fashion Search v${this.version} - Enhanced Product Recognition (Fixed)`;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('🎯 Fashion Search v11.1.1 초기화 완료!');
    console.log('🐛 버그 수정:');
    console.log('  ✅ HTML-JS ID 매칭 문제 해결');
    console.log('  ✅ 모드 전환 기능 수정');
    console.log('  ✅ 버튼 이벤트 리스너 수정');
